...
# Final version with both MQTT Realtime and HTTP Daily Stats sensors
...
# Import CoordinatorEntity và DataUpdateCoordinator
...
try:
    # Import đầy đủ các KEY và Coordinator
    from .const import (
        DOMAIN, _LOGGER, CONF_DEVICE_SN, CONF_DEVICE_NAME,
        SIGNAL_UPDATE_FORMAT, # Signal cho MQTT
        # Import các KEY tức thời
    ...
    class LumentreeStatsCoordinator: pass # Giả lập


# --- Định nghĩa mô tả Sensor TỨC THỜI (MQTT) ---
REALTIME_SENSOR_DESCRIPTIONS: tuple[SensorEntityDescription, ...] = (
    ...
    SensorEntityDescription(key=KEY_BATTERY_POWER, name="Battery Power (Abs)", native_unit_of_measurement=UnitOfPower.WATT, device_class=SensorDeviceClass.POWER, state_class=SensorStateClass.MEASUREMENT, icon="mdi:battery"),
...
# --- Định nghĩa mô tả Sensor THỐNG KÊ NGÀY (HTTP API) ---
STATS_SENSOR_DESCRIPTIONS: tuple[SensorEntityDescription, ...] = (
    SensorEntityDescription(
        key=KEY_DAILY_PV_KWH, # Key này khớp với key trong coordinator_stats
        name="PV Generation Today",
        native_unit_of_measurement=UnitOfEnergy.KILO_WATT_HOUR,
        device_class=SensorDeviceClass.ENERGY,
        state_class=SensorStateClass.TOTAL, # Tổng của 1 ngày
...
async def async_setup_entry(
    ...
    try:
        # Lấy coordinator từ hass.data
        entry_data = hass.data[DOMAIN][entry.entry_id]
        coordinator_stats: LumentreeStatsCoordinator = entry_data.get("coordinator_stats")
    except KeyError:
        _LOGGER.error(f"Lumentree data not found in hass.data for entry {entry.entry_id}. Setup failed?")
        return

    device_sn = entry.data[CONF_DEVICE_SN]
    device_name = entry.data[CONF_DEVICE_NAME]

    # Tạo thông tin thiết bị chung
    ...
    _LOGGER.info(f"Adding {len(REALTIME_SENSOR_DESCRIPTIONS)} real-time sensors for {entry.title}")

    # --- Tạo các Sensor Thống kê Ngày (HTTP API) ---
    if coordinator_stats: # Chỉ tạo nếu coordinator tồn tại
        initial_stats_data = coordinator_stats.data or {}
        added_stats_keys = set()

        for description in STATS_SENSOR_DESCRIPTIONS:
            # Chỉ thêm nếu key có trong dữ liệu ban đầu và chưa được thêm
            if description.key in initial_stats_data and description.key not in added_stats_keys:
                 ...
                 _LOGGER.debug(f"Adding stats sensor {description.key} with initial value.")
            # else: # Bỏ log này để tránh spam
            #     _LOGGER.debug(f"Skipping stats sensor {description.key}, not found in initial coordinator data.")

        _LOGGER.info(f"Adding {len(added_stats_keys)} daily stats sensors for {entry.title}")
    else:
    ...
    if entities_to_add:
        ...
        _LOGGER.warning("No sensors were added for Lumentree.")


# --- Lớp Sensor Real-time (MQTT) ---
class LumentreeMqttSensor(SensorEntity):
    """Representation of a Lumentree real-time sensor using dispatcher."""
...
# --- LỚP SENSOR THỐNG KÊ NGÀY (HTTP API) MỚI ---
class LumentreeDailyStatsSensor(CoordinatorEntity[LumentreeStatsCoordinator], SensorEntity):
    """Representation of a Lumentree daily statistics sensor using Coordinator."""

    _attr_has_entity_name = True # Tên entity sẽ là tên description
    ...
    def __init__(
        ...
        """Initialize the sensor."""
        super().__init__(coordinator) # Gọi init của CoordinatorEntity
        self.entity_description = description
        self._device_sn = coordinator.device_sn # Lấy SN từ coordinator
        self._attr_unique_id = f"{self._device_sn}_{description.key}" # Tạo unique ID
        self._attr_device_info = device_info
        self._attr_attribution = "Data fetched via Lumentree HTTP API"

        # Cập nhật trạng thái ban đầu từ dữ liệu coordinator đã có
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        self.async_write_ha_state() # Cập nhật state trong HA
    ...
    def _update_state_from_coordinator(self) -> None:
        ...
        # Gán giá trị hoặc None
        if isinstance(value, (int, float)):
             self._attr_native_value = value
        else:
             if value is not None:
                  _LOGGER.warning(f"Unexpected data type '{type(value)}' for stats sensor {self.unique_id}, expected float or int.")
             self._attr_native_value = None

    # Property `available` được kế thừa từ CoordinatorEntity
